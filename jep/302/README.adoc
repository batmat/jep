...
We will configure Jenkins in a way that physically separate as much as possible the things that must be snapshotted, and things that will not (see <<data_segregation>> for details).
...
Basically, instead of the now usual `/var/jenkins_home`, we will introduce two subdirectories:
